...
自动导览逐站飞行，镜头与观察目标到达建筑后才切换对应信息，停留约 4 秒后进入下一站。点击模型、从列表选择或通过建筑深链接查看时，复用同一套建筑信息气泡，仅显示建筑上方气泡。信息气泡锚定当前建筑的世界坐标，并根据对应建筑及屋顶的实际包围盒抬升到建筑上方，随地图移动；同步显示建筑名称、现有说明与资料入口，手机无需展开侧栏即可阅读。暂停保留当前信息，退出后重新进入从首站开始。
