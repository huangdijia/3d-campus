...
自动导览逐站飞行，镜头与观察目标到达建筑后才切换对应信息，停留约 4 秒后进入下一站。点击模型、从列表选择或通过建筑深链接查看时，复用同一套建筑信息气泡，并保留选中建筑的定位环。信息气泡锚定当前建筑的世界坐标，并根据对应建筑及屋顶的实际包围盒抬升到建筑上方，随地图移动；同步显示建筑名称、现有说明与资料入口，手机无需展开侧栏即可阅读。暂停保留当前信息，退出后重新进入从首站开始。

## PWA 与生产静态资源

应用清单提供独立窗口模式和桌面/主屏幕图标。Service Worker 仅在生产安全上下文注册，预热当前页面实际请求过的资源，校园模型按数量和字节上限淘汰；未访问资料仍需联网。生产 Worker 使用 `ASSETS` 静态绑定兑现 Vinext 的资源响应，确保清单、Service Worker、脚本、图标和模型返回真实文件；对应配置见 `vite.config.ts`。参考 [Cloudflare 静态资源绑定](https://developers.cloudflare.com/workers/static-assets/binding/)，实际安装与离线结果见 [PWA 验证](pwa.md)。
