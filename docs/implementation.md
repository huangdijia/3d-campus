...
全国高校列表点击后，镜头先飞至学校位置，到达后进入校园；飞行期间可取消、拖动地图或改选学校。减少动态效果偏好、无 WebGL 或场景加载失败时直接进入。校园页返回按钮直接回到列表，保留筛选和地图视角。学校介绍、真实校徽和官网整合到校园侧栏，资料不足的学校保留信息页。旧学校详情 URL 自动转为统一校园地址；筛选、建筑和游览模式同步 URL，可刷新和分享。地区筛选使用全国与七大区玻璃弹出面板，旧省份参数兼容转换为所属大区，URL 优先于历史存储。页面无顶部导航栏。手机默认全屏地图，仅保留底部信息条，点击或上滑展开面板，选择学校、建筑或游览模式后收起。鸟瞰、自动导览与漫游为互斥模式，自动导览另设暂停/继续。
...
自动导览逐站飞行，镜头与观察目标到达建筑后才切换对应信息，停留约 4 秒后进入下一站。点击模型、从列表选择或通过建筑深链接查看时，复用同一套建筑信息气泡，仅显示建筑上方气泡。左侧始终保留建筑搜索和列表，点击列表项仅定位建筑并高亮当前项，不切换为重复详情面板；全国与校园侧栏均不再显示底部统计或几何预览提示。信息气泡锚定当前建筑的世界坐标，并根据对应建筑及屋顶的实际包围盒抬升到建筑上方，随地图移动；仅在有具体建筑介绍时显示正文与资料入口；现有统一的 OpenStreetMap 来源、估算高度和待核验说明不作为建筑介绍，气泡只显示名称，宽度随标题适配。手机无需展开侧栏即可阅读。暂停保留当前信息，退出后重新进入从首站开始。
