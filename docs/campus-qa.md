...
## 2026-09-06 无具体介绍时仅显示建筑名称

核查当前 7,728 条 POI，仅有两种通用说明：7,561 条采用估算高度、167 条使用来源高度。它们是几何来源说明，不是建筑介绍。气泡将这两种说明隐藏，并同时隐藏资料入口；没有具体介绍时只显示名称，标题卡按文字宽度缩放。未来具体建筑介绍仍可显示正文与来源。最新列表及名称气泡浏览器回归见 [校园侧栏回归](campus-sidebar-qa.md)。以下各节保留前一轮实现的历史截图与检查结果。
...
修复原来每 6 秒从 `path[index]` 飞向 `path[index+1]`、但始终报告起点索引的问题。现在逐站飞行，相机和观察目标均进入 0.1 m 到达范围后精确落点并更新索引，停留 4 秒再前往下一站。首站到达前索引为 -1，不提前显示建筑卡；暂停保留当前站点，退出导览重置进度。

Chrome 390×844 连续两站回归通过：

| 到达站点 | 信息卡 | 实测 OrbitControls target | 资料链接 |
|---|---|---|---|
| 清华学堂 | 清华学堂 · 1 / 8 | `[-105.96575990982967, 15, 34.029738999852555]` | OSM way 83805067 |
| 新清华学堂 | 新清华学堂 · 2 / 8 | `[276.88710893111215, 15, 146.07983822266476]` | OSM way 158112141 |

两站 target 均与对应 POI.position 完全一致；标题、序号、来源链接随到站更新。描述均匹配对应 POI 数据，两条记录目前使用相同的待核验说明。第二站暂停 1.5 秒，卡片全文、相机和 target 完全不变，渲染帧计数保持 609；控制台 0 errors。

另从第二站切回鸟瞰后重新开启导览：初始卡片不显示，到达后回到“清华学堂”，target 再次与首站坐标完全一致，确认退出复位而非延续旧索引。

`output/playwright/campus-tour-arrival-mobile.png` 是第二站暂停时的实际手机截图，已视觉确认建筑信息卡与底部控制均可见。校园场景 scoped lint 和 TypeScript 检查通过，完整构建由站点负责者执行。

## 2026-09-06 建筑上方导览气泡

根据随后澄清，固定位置的信息卡已被建筑上方的气泡替换；前一节 `campus-tour-arrival-mobile.png` 仅是旧版记录。当前气泡由 Drei Html 在世界坐标渲染，无 distanceFactor，屏幕文字大小不随观察距离改变。锚点保留对应 POI 的 x/z，高度为 `max(POI.y, 对应建筑及屋顶模型最高点) + 2 m`。包围盒读取合并网格的该建筑三角面范围，排除邻楼，并应用模型世界变换。
...
|---|---|
| 首站到达前 | PASS，不显示气泡 |
| 清华学堂与新清华学堂 | PASS，名称和资料链接随到站更新；气泡尖角与对应世界锚点投影误差均小于 0.001 px |
| 建筑高度 | 两站实际模型最高点约 12.2 m，POI.y=15，气泡 anchor.y=17；未使用整校包围盒高度 |
| 固定屏幕尺寸 | 两站及拖动后气泡宽度均为 230 px |
| 暂停 | PASS，1.2 秒内气泡位置、信息和视点不变，帧数保持 611 |
| 暂停后拖动地图 | PASS，气泡尖角从 (195, 418.15) 移至 (227.89, 448.04)，继续贴合该建筑投影，误差小于 0.001 px |

实际截图已视觉检查：`output/playwright/campus-tour-bubble-stop-1.png`、`campus-tour-bubble-stop-2.png`、`campus-tour-bubble-pan.png`。它们分别显示两站建筑上方的气泡，以及拖动后的跟随位置，均未固定在屏幕角落。

新增几何测试覆盖合并网格索引范围、邻楼排除、独立屋顶高度与世界变换；总计 8/8 测试通过。修改的校园文件 scoped lint、TypeScript 通过，浏览器正常交互 0 errors。此补充只覆盖清华两站的气泡行为，不代表逐校内容核验。
