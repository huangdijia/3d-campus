...
打开启动命令打印的本地地址。点击高校列表后，地图镜头先飞至学校位置，再进入校园；可在途中取消或改选学校。学校介绍与官网位于校园侧栏；返回直接回到高校列表。地区面板提供全国与七大区筛选，可与搜索、985 / 211 标签组合。手机默认显示全屏地图，通过底部信息条展开搜索与校园信息。全国地图左键拖动平移，滚轮缩放，右键拖动旋转；触屏单指平移、双指缩放。学校列表支持键盘操作与 `/` 搜索快捷键。点击建筑列表直接定位建筑，左侧保留列表；自动导览到站后停留约 4 秒。建筑名称气泡随视角移动，只有具备具体介绍时才显示正文。

应用支持 PWA，可通过浏览器的“安装应用”或“添加到主屏幕”入口使用独立窗口。离线功能仅缓存已访问内容，未下载的校园需要联网；生产环境注册 Service Worker，开发服务器不启用。详情见 [PWA 说明](docs/pwa.md)。
