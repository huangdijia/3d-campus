...
打开启动命令打印的本地地址。点击高校直接进入校园，学校介绍与官网位于校园侧栏；返回直接回到高校列表。手机默认显示全屏地图，通过底部信息条展开搜索与校园信息。全国地图左键拖动平移，滚轮缩放，右键拖动旋转；触屏单指平移、双指缩放。学校列表支持键盘操作与 `/` 搜索快捷键。自动导览到达建筑后，在对应建筑上方显示名称和资料气泡，随视角移动，并停留约 4 秒。
